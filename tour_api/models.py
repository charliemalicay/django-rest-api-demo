...
class PackagePermission(models.Model):
    ...
    def __str__(self):
        if self.is_owner:
            message = "can write to"
        else:
            message = "cannot write to"

        return f"{self.user.username} ({self.user.id}) {message} {self.package.name} ({self.package.id})"
    ...
    def set_can_write(cls, user, package):
        obj, created = cls.objects.get_or_create(user=user, package=package, default={'is_owner': True})
